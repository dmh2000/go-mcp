...
func (s *Server) handleInitializeRequest(id mcp.RequestID, payload []byte) ([]byte, error) {
	...
	result := mcp.InitializeResult{
		...
		Instructions: "Welcome to the Go MCP Example Server! The 'random_data' resource, 'ping' tool, and 'sqirvy_query' prompt are available.", // Optional, updated instructions
	}
...
func (s *Server) handleListResources(id mcp.RequestID) ([]byte, error) {
	...
	resourcesList := []mcp.Resource{exampleFileResource}
...
func (s *Server) handleReadResource(id mcp.RequestID, payload []byte) ([]byte, error) {
	...
	if resourceErr != nil {
		s.logger.Printf("Error reading resource URI '%s': %v", params.URI, resourceErr)
